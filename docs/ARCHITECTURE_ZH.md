...
| **Phase 3: 性能优化 (Sparse)** | ✅ 已完成 | 全链路零分配、SBE 直接解码、硬件预取加速、$O(1)$ 侵入式触发链表、缓存行对齐。 |
| **Phase 4: 快照硬化与分布式** | 🚧 进行中 | Jemalloc 屏障加固、流式快照 I/O、分布式定序器 (Sequencer) 整合方案。 |
...
MT-Engine 对核心数据结构进行了严格的 ABI 对齐。以 `OrderData` 为例：

*   **Line 0 (Hot)**：将成交价格、剩余数量、过期时间、触发价等撮合热路径字段集中在前 64 字节。
*   **Line 1 (Cold)**：将 OrderID、UserID、时间戳等审计类字段隔离在后 64 字节，避免 False Sharing。

```
Cache Line (64 bytes):
┌────────────────────────────────────┬────────────────────────────────────┐
│ HOT DATA (0..63 bytes)             │ COLD DATA (64..127 bytes)          │
│ qty(8) + price(8) + trigger(8) ... │ order_id(8) + user_id(8) + ...     │
├────────────────────────────────────┼────────────────────────────────────┤
│ 128-byte Alignment (Single Slab)   │ (Prevents cache line splitting)    │
└────────────────────────────────────┴────────────────────────────────────┘
```

### 5.4 条件单侵入式触发池 (Zero-Allocation)

在 Sparse 模式下，条件单（Stop/TP）的触发不再使用 `Vec`，而是通过 `Slab` 与侵入式双向链表实现：

1.  **$O(1)$ 复杂度**：通过双向指针，撤销任意位置的条件单均为常数时间。
2.  **物理零分配**：触发循环中不涉及 heap 分配，所有节点通过预分配池 (Node Pool) 回收利用。
3.  **硬件预取**：在遍历链表时，提前将 `OrderData` 预取至 L1 缓存（x86: `_mm_prefetch`, ARM: `prfm`）。
...
pub struct OrderData {
    ...
    pub flags: OrderFlags,      // 标志位 (Post-Only/Iceberg)
    pub visible_qty: Quantity,  // 冰山可见量
    pub peak_size: Quantity,    // 冰山峰值
    pub expiry: Timestamp,      // 过期时间 (Hot: 用于 GTD 校验)
    pub trigger_price: Price,   // 触发价 (Hot: 用于 Stop 单扫描)
...
在 `execute_amend` 过程中，如果修改价格触发了撮合：

1.  **乐观执行**：尝试将订单从簿中取出并进行匹配。
2.  **错误回滚 (Rollback)**：如果撮合过程发生错误（如违反 `PostOnly` 约束），系统会自动将原始订单数据重新挂回订单簿。
3.  **触发一致性**：改单导致的 LTP (最新成交价) 变更会自动触发连锁条件单扫描，确保状态机完整性。

### 9.3 性能基准特征

根据最新基准测试，系统的性能表现如下：
*   **缓存行增益**：对齐后的热路径耗时下降约 **80%**。
*   **分配器收益**：在混合负载下，`jemalloc` 的内存管理效率比系统默认分配器提升约 **10-15%**。
...
2. **Multi-Asset Support**：单引擎实例多交易对支持。
3. **RDMA 极速快照**：利用用户态网络栈进一步降低快照干扰。
